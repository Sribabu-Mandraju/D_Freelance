...
import { User, Code, Briefcase, GraduationCap, TrendingUp, Mail, Github, Linkedin, Globe, Phone, MapPin, ExternalLink, Zap, Shield, Cpu, Database, Edit, Save, X, Plus, Trash2 } from 'lucide-react'
...
function Portfolio() {
  ...
  const updateStatus = (id, field, value) => {
    setEditStatus(prev => prev.map(status => 
  ...
  const updateTech = (index, field, value) => {
    setEditTech(prev => prev.map((tech, i) => 
  ...
  return (
    <div className="min-h-screen  bg-gradient-to-br from-gray-900 via-black to-gray-900 relative overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row flex-1 overflow-hidden pt-20 sm:px-4 lg:px-20 relative z-10">
        ...
        {/* Main Content */}
        <main className="flex-1 p-3 sm:p-4   lg:p-6 overflow-y-auto" aria-label="About content">
          {/* Hero Section */}
        ...
        <aside className="w-full lg:w-80 p-4 lg:p-6 space-y-6 overflow-y-auto order-last lg:order-none" aria-label="About sidebar">
          {/* Quick Contact */}
          ...
          <div className="bg-black/40 backdrop-blur-xl border border-purple-500/20 rounded-2xl p-4 sm:p-6 shadow-2xl shadow-purple-500/10 relative overflow-hidden">
            ...
            <div className="space-y-3 text-sm">
              {(isEditingStatus ? editStatus : currentStatus).map((status) => (
                <div key={status.id} className={`flex items-center  gap-3 p-3 bg-gradient-to-r from-${status.color}-500/10 to-${status.color === 'green' ? 'emerald' : status.color === 'blue' ? 'cyan' : 'pink'}-500/10 rounded-lg border border-${status.color}-500/20 relative`}>
                  
                  <div className="relative">
                    <div className={`w-3 h-3 bg-${status.color}-400 rounded-full ${status.active ? 'animate-ping opacity-75' : 'animate-pulse'}`}></div>
                    {status.active && <div className={`absolute inset-0 w-3 h-3 bg-${status.color}-400 rounded-full`}></div>}
                  </div>
          ...
          <div className="bg-black/40 backdrop-blur-xl border border-cyan-500/20 rounded-2xl p-4 sm:p-6 shadow-2xl shadow-cyan-500/10 relative overflow-hidden">
            ...
            <div className="space-y-3">
              {(isEditingTech ? editTech : techHighlights).map((tech, index) => (
                <div key={index} className="flex flex-row w-full items-center justify-between p-2 bg-gradient-to-r from-gray-800/30 to-gray-900/30 rounded-lg border border-gray-700/30 relative">
