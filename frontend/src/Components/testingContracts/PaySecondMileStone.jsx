...
import { usePaySecondMilestone } from "../../interactions/ProposalManager_interactions";
import { baseSepolia } from "wagmi/chains";
import toast from "react-hot-toast";

function PaySecondMilestone() {
  ...
  // Handle pay second milestone
  const handlePaySecondMilestone = async () => {
    if (!isConnected) {
    ...
    try {
      console.log("Calling paySecondMilestone with proposalId:", proposalId);
      hasShownToast.current = false; // Reset toast flag
      await paySecondMilestone(proposalId);
    } catch (err) {
      console.error("Pay second milestone error:", err);
  ...
  useEffect(() => {
    ...
    if (isPending && !hasShownToast.current) {
      toastId = toast.loading("Processing second milestone payment...");
      hasShownToast.current = true;
    } else if (isConfirming && !hasShownToast.current) {
      toastId = toast.loading("Confirming second milestone payment...");
      hasShownToast.current = true;
    } else if (isConfirmed && !hasShownToast.current) {
      toastId = toast.success("Second milestone payment successful!");
      hasShownToast.current = true;
  ...
  return (
    <div className="p-5 max-w-md mx-auto bg-gray-800 rounded-lg shadow-lg">
      <h3 className="text-xl font-semibold mb-4 text-gray-100">Pay Second Milestone</h3>
      <div className="mb-4">
      ...
      <button
        onClick={handlePaySecondMilestone}
        disabled={
          isPending ||
          isConfirming ||
          !hasEnoughGas ||
      ...
      >
        {isPending || isConfirming ? "Processing..." : "Pay Second Milestone"}
      </button>
    </div>
  );
}

export default PaySecondMilestone;