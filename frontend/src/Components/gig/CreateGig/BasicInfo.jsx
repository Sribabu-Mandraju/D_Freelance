  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
        <div>
          <label className="block text-sm font-medium text-cyan-300 mb-2 tracking-wide">Hourly Pay (USD) *</label>
        ...
        <div>
          <label className="block text-sm font-medium text-cyan-300 mb-2 tracking-wide">Custom UI *</label>
          <select
            value={pkg.custom_ui}
...
export default function BasicInfo({ formData, setFormData, packageData, setPackageData }) {
  ...
  const handleGigImageUpload = async () => {
    ...
    try {
      const res = await axios.post("https://api.cloudinary.com/v1_1/dd33ovgv1/image/upload", uploadFormData)
  ...
  useEffect(() => {
    ...
    if (Array.isArray(formData.images) && formData.images.length > 0) {
      const showcaseImages = formData.images.filter((img) => img.url !== formData.gigimage)
      initialPreviews.push(...showcaseImages.map((img) => ({ url: img.url, type: "showcase", uploaded: true })))
    }
    ...
    setAllImagePreviews((prev) => {
      ...
      const uploadedChanged =
        ...
        !currentUploaded.every((p, i) => initialPreviews[i] && p.url === initialPreviews[i].url)
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-950 via-slate-900 to-gray-950 text-white p-4 sm:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8 mb-8">
        <div className="space-y-2">
          <label className="block text-sm sm:text-base font-medium text-cyan-300 mb-2 tracking-wide">Gig Title *</label>
          <input
            type="text"
        ...
        <div className="space-y-2">
          <label className="block text-sm sm:text-base font-medium text-cyan-300 mb-2 tracking-wide">Username *</label>
          <input
            type="text"
        ...
        <div className="space-y-2">
          <label className="block text-sm sm:text-base font-medium text-cyan-300 mb-2 tracking-wide">Category *</label>
          <div className="relative group">
        ...
        <div className="space-y-2">
          <label className="block text-sm sm:text-base font-medium text-cyan-300 mb-2 tracking-wide">Price *</label>
          <div className="relative group">
          ...
          <div className="flex relative z-10">
            {["basic", "standard", "pro"].map((pkgName) => (
              ...
              >
                ...
                <span className="relative z-10">{pkgName.charAt(0).toUpperCase() + pkgName.slice(1)}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="mb-8">
        <label className="block text-sm sm:text-base font-medium text-cyan-300 mb-3 tracking-wide">Gig Image</label>
      ...
      <div className="mb-8">
        ...
        {allImagePreviews.length > 0 && (
          <div className="space-y-4">
            <span className="block text-sm text-cyan-300 font-medium tracking-wide">Showcase Images Preview:</span>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-2">
                      ...
                      >
                        ...
                        <span className="hidden sm:inline">
                          {isReplacing && replaceIndex === index ? "Replacing..." : "Replace"}
                        </span>
                      </button>
      ...
      <div className="space-y-3">
        <label className="block text-sm sm:text-base font-medium text-cyan-300 tracking-wide">Gig Description *</label>
        <textarea
