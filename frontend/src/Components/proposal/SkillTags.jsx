...
export default function SkillsTags({
  ...
  const addCustomSkill = () => {
    if (
      customSkill.trim() &&
      !formData.skills_requirement.includes(customSkill.trim())
    ) {
    ...
    <div className="md:p-8 p-4">
      <div className="mb-6">
        ...
        <p className="text-gray-300">
          Select required Web3 skills and add relevant tags
        </p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div>
                ...
                {formData.skills_requirement.map((skill) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="ml-2 hover:bg-cyan-600 rounded-full p-1 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      ×
                    </button>
                  </span>
            ...
            <div className="flex flex-wrap gap-2">
              {popularSkills.map((skill) => (
                <button
                  ...
                  disabled={
                    formData.skills_requirement.includes(skill) || isSubmitting
                  }
          ...
          {errors.skills_requirement && (
            <p className="mt-2 text-xs md:text-sm text-red-400 animate-pulse">
              {errors.skills_requirement}
            </p>
          )}
        </div>
                ...
                {formData.tags.map((tag) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={isSubmitting}
                      className="ml-2 hover:bg-purple-600 rounded-full p-1 transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      ×
                    </button>
                  </span>
      ...
      <div className="flex justify-between mt-8 pt-6 border-t border-gray-700">
        <button
          onClick={prevStep}
          disabled={isSubmitting}
          className={`md:px-8 px-4 md:py-3 py-2 bg-slate-700 text-gray-300 font-semibold rounded-xl transform transition-all duration-200 ${
            isSubmitting
        ...
        <button
          ...
          disabled={isSubmitting}
          className={`md:px-8 px-4 md:py-3 py-2 bg-gradient-to-r from-cyan-500 to-blue-500 text-white font-semibold rounded-xl transform transition-all duration-200 shadow-lg shadow-cyan-500/25 ${
            isSubmitting
              ? "opacity-50 cursor-not-allowed"
