...
import { getMessages, subscribeToMessages, unsubscribeFromMessages } from "../../store/ChatApplicationSlice/ChatAppSlice";
const ChatContainer = () => {
  const {
    messages,
    isMessagesLoading,
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-900/40 backdrop-blur-sm">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin scrollbar-thumb-cyan-400/20 scrollbar-track-transparent">
        {authUser && messages.map((message, index) => {
          ...
          return (
            ...
            >
              <div className={`flex items-end gap-3 max-w-xs lg:max-w-md xl:max-w-lg ${
                ...
                <div className={`flex flex-col ${isOwnMessage ? "items-end" : "items-start"}`}>
                  {/* Timestamp */}
                  <div className={`text-xs text-slate-400 mb-1 ${isOwnMessage ? "mr-2" : "ml-2"}`}>
