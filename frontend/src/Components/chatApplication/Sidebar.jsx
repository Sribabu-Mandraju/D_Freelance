...
const Sidebar = () => {
  const { users, selectedUser, isUsersLoading } = useSelector((state)=>state.chatApp);
    ...
    <aside className="h-full w-full lg:w-80 bg-slate-900/60 backdrop-blur-sm border-r border-cyan-400/30 flex flex-col transition-all duration-300 shadow-lg">
      ...
      <div className="border-b border-cyan-400/30 w-full p-6 bg-gradient-to-r from-slate-800/50 to-slate-700/50">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-xl hidden lg:block bg-gradient-to-r from-cyan-400 to-purple-400 bg-clip-text text-transparent">Contacts</span>
        </div>
        ...
        <div className="mt-4 hidden lg:flex items-center justify-between">
          <label className="cursor-pointer flex items-center gap-3 group">
            <div className="relative">
              <input
                ...
                className="sr-only"
              />
              <div className={`w-12 h-6 rounded-full transition-all duration-300 ${showOnlineOnly ? 'bg-gradient-to-r from-cyan-500 to-purple-500' : 'bg-slate-600'}`}>
                <div className={`w-5 h-5 bg-white rounded-full shadow-lg transform transition-transform duration-300 mt-0.5 ${showOnlineOnly ? 'translate-x-6' : 'translate-x-0.5'}`}></div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-slate-400">{onlineUsers?.length || 0} online</span>
          </div>
        </div>
      </div>
      ...
      <div className="overflow-y-auto w-full py-2 px-4 scrollbar-thin scrollbar-thumb-cyan-400/20 scrollbar-track-transparent">
        ...
        {filteredUsers.length === 0 && (
          <div className="text-center py-8">
            ...
            <div className="text-sm text-slate-500">Try adjusting your filter settings</div>
          </div>
        )}
      </div>
    </aside>
